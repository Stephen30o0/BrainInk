...
// Additional imports for enhanced PDF processing
...
if (process.env.GOOGLE_API_KEY) {
    ...
    // Load grade cache for enhanced PDF grading
    loadGradeCache();
} else {
...
const loadGradeCache = async () => {
    try {
        ...
        console.log('📋 No existing grade cache found, starting fresh.');
        gradeCache = {};
    }
};
...
const saveGradeCache = async () => {
    try {
        await fsPromises.writeFile(GRADE_CACHE_PATH, JSON.stringify(gradeCache, null, 2));
    } catch (error) {
...
// Get cached grade
const getCachedGrade = (hash) => {
    return gradeCache[hash] || null;
};

// Set cached grade
const setCachedGrade = (hash, result) => {
    gradeCache[hash] = result;
    // Save cache asynchronously
    saveGradeCache().catch(console.error);
};
