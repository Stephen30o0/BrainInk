...
interface FullScreenCalendarProps {
    ...
    onDayClick?: (day: Date, events: Event[]) => void
}
...
export function FullScreenCalendar({
    ...
    function handleDayClick(day: Date) {
        setSelectedDay(day)
        const dayEvents = data.filter((date) => isSameDay(date.day, day)).flatMap(d => d.events)
    ...
    const getEventTypeColor = (type?: string) => {
        switch (type) {
            ...
            case 'class':
                return 'bg-blue-500/20 border-blue-500/40 text-blue-700'
            case 'exam':
                return 'bg-purple-500/20 border-purple-500/40 text-purple-700'
            case 'meeting':
                                                ...
                                                <div key={day.day.toString()} className="space-y-1.5">
                                                    {day.events.slice(0, 2).map((event) => (
                                                        <div
                                                            ...
                                                            className={cn(
                                                                "flex flex-col items-start gap-1 rounded-lg border p-2 text-xs leading-tight cursor-pointer hover:opacity-80 transition-opacity",
                                                        ...
                                                        >
                                                            ...
                                                            <p className="leading-none text-xs opacity-75">
                                                                {event.time}
                                                            </p>
                                                        </div>
                                                    ))}
                                                                ...
                                                                event.type === 'assignment' ? 'bg-red-500' :
                                                                    event.type === 'class' ? 'bg-blue-500' :
                                                                        event.type === 'exam' ? 'bg-purple-500' :
                                                                            event.type === 'syllabus' ? 'bg-orange-500' :
                                                                                'bg-muted-foreground'
                                                            )}
                                                        />
                                                    ))}
